...
class GoogleRecipeSchema {
  ...
  async getDescription() {
    return this.setDefaultValuesIfEmpty(this.recipe.summary, this.recipe.title);
  }
  ...
  async getPrepTime() {
    return this.setDefaultValuesIfEmpty(this.recipe.preparationMinutes, 15);
  }

  async getCookTime() {
    return this.setDefaultValuesIfEmpty(this.recipe.cookingMinutes, 40);
  }

  async getTotalTime() {
    const sumTime = this.getPrepTime() + this.getCookTime();
    return this.setDefaultValuesIfEmpty(this.recipe.readyInMinutes, 40);
  }
  ...
  async getRatingCount() {
    return this.setDefaultValuesIfEmpty(this.recipe.aggregateLikes, 5);
  }

  async getServings() {
    return this.setDefaultValuesIfEmpty(this.recipe.servings, 6);
  }
  ...
  async getInstructions() {
    ...
    for (let i = 0; i < this.recipe.analyzedInstructions.length; i++) {
      for (
        let y = 0;
        y < this.recipe.analyzedInstructions[i].steps.length;
        y++
      ) {
        instructions.push({
          '@type': 'HowToStep',
